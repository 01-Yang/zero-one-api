...
func TestListPlazaGroups_GroupModelPricingOverridesChannelAndLegacyGroupPricing(t *testing.T) {
	channelTokenInput := 3e-6
	groupTokenInput := 1.25e-6
	ignoredGroupTokenTier := 9e-6
	channelImagePrice := 0.2
	legacyImagePrice := 0.134
	groupImagePrice := 0.0
	channels := []Channel{{
		ID: 1, Name: "priced", Status: StatusActive, GroupIDs: []int64{10},
		ModelPricing: []ChannelModelPricing{{
			Platform: PlatformOpenAI, Models: []string{"gpt-5"}, BillingMode: BillingModeToken,
			InputPrice: &channelTokenInput,
		}, {
			Platform: PlatformOpenAI, Models: []string{"gpt-image-2"}, BillingMode: BillingModeImage,
			PerRequestPrice: &channelImagePrice,
		}},
	}}
	groups := []Group{{
		ID: 10, Name: "group-priced", Platform: PlatformOpenAI, RateMultiplier: 1,
		ImagePrice1K: &legacyImagePrice,
		ModelPricing: []ChannelModelPricing{{
			Platform: PlatformOpenAI, Models: []string{"gpt-5"}, BillingMode: BillingModeToken,
			InputPrice: &groupTokenInput,
			Intervals:  []PricingInterval{{MinTokens: 200000, InputPrice: &ignoredGroupTokenTier}},
		}, {
			Platform: PlatformOpenAI, Models: []string{"gpt-image-2"}, BillingMode: BillingModeImage,
			Intervals: []PricingInterval{{TierLabel: "1K", PerRequestPrice: &groupImagePrice}},
		}},
	}}
	...
	require.InDelta(t, groupTokenInput, *byName["gpt-5"].Pricing.InputPrice, 1e-12)
	require.Empty(t, byName["gpt-5"].Pricing.Intervals, "runtime ignores token intervals from a group model card")
	imagePricing := byName["gpt-image-2"].Pricing
	require.NotNil(t, imagePricing)
	require.Len(t, imagePricing.Intervals, 1)
	require.InDelta(t, groupImagePrice, *imagePricing.Intervals[0].PerRequestPrice, 1e-12)
	require.Nil(t, imagePricing.PerRequestPrice, "group model card must replace, not merge with, channel pricing")

	// Plaza must not mutate repository-owned channel/group pricing slices.
	require.InDelta(t, channelTokenInput, *channels[0].ModelPricing[0].InputPrice, 1e-12)
	require.InDelta(t, channelImagePrice, *channels[0].ModelPricing[1].PerRequestPrice, 1e-12)
	require.InDelta(t, groupTokenInput, *groups[0].ModelPricing[0].InputPrice, 1e-12)
	require.Len(t, groups[0].ModelPricing[0].Intervals, 1)
}

func TestListPlazaGroups_LongContextDisabledShowsFirstChannelTierAsFlatPricing(t *testing.T) {
	flatInput := 3e-6
	firstInput := 4e-6
	secondInput := 8e-6
	maxFirst := 200000
	channel := Channel{
		ID: 1, Name: "tiered", Status: StatusActive, GroupIDs: []int64{10},
		ModelPricing: []ChannelModelPricing{{
			Platform: PlatformAnthropic, Models: []string{"claude-sonnet"}, BillingMode: BillingModeToken,
			InputPrice: &flatInput,
			Intervals: []PricingInterval{{
				MinTokens: 0, MaxTokens: &maxFirst, InputPrice: &firstInput,
			}, {
				MinTokens: 200000, InputPrice: &secondInput,
			}},
		}},
	}
	group := Group{
		ID: 10, Name: "no-long-context", Platform: PlatformAnthropic, RateMultiplier: 1,
		LongContextPricingEnabled: false,
	}

	out, err := newPlazaChannelService([]Channel{channel}, []Group{group}, nil).ListPlazaGroups(context.Background())
	...
	require.InDelta(t, firstInput, *pricing.InputPrice, 1e-12)
	require.Len(t, channel.ModelPricing[0].Intervals, 2, "display projection must not mutate channel pricing")
}
