...
- 常规产品工作仅限品牌、Public Site、Console 主题和部署边缘层，不改变 Sub2API 的业务范围、权限或兑换规则。仅当稳定上游版本存在会阻断生产正确性的已确认缺陷时，才允许有测试、精确文件白名单和退出条件的临时后端 backport；下一个包含等价修复的稳定 tag 发布后必须收敛回上游实现并删除临时权限。
