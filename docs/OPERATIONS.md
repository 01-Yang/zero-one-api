...
is the `ui-approved-2026-08-20-r4` tag at commit
`c4a0744ecd8099c24b755318da0f2bab5b09f2f3`. The original
...
- For releases based on v0.1.178, confirm `schema_migrations` contains `221_group_model_pricing.sql`, `222_group_pricing_auth_cache_invalidation.sql`, `222_group_usage_daily_rollups.sql`, `223_group_usage_rollup_timezone.sql`, `224_user_platform_quotas_add_cn_providers.sql`, `225_backfill_codex_fingerprint_seed.sql`, `225_channel_model_time_pricing.sql`, `226_channel_monitor_quota_mode.sql`, and `224_add_public_visible_to_announcements.sql`. Verify `groups.long_context_pricing_enabled` is non-null with default `true`, `groups.model_pricing` is nullable `jsonb`, the group auth-cache trigger function compares both columns, `announcements.public_visible` is non-null with default `false`, and `idx_announcements_public_active_id` is valid and ready.
