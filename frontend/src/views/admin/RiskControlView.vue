          ...
          <div class="grid grid-cols-1 gap-6 p-6 xl:grid-cols-[minmax(0,360px)_1fr]">
            <div class="space-y-4">
              ...
              <div class="grid grid-cols-2 gap-3">
                ...
                <div class="rounded-lg bg-zo-signal-50 p-4 dark:bg-zo-signal-900/10">
                  <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('admin.riskControl.idleWorkers') }}</p>
                  <p class="mt-2 text-2xl font-semibold text-zo-signal-700 dark:text-zo-signal-300">{{ status?.idle_workers ?? configForm.worker_count }}</p>
                </div>
                <div class="rounded-lg bg-gray-50 p-4 dark:bg-dark-700/50">
                  ...
                  <tr v-for="row in logs" :key="row.id" class="hover:bg-gray-50 dark:hover:bg-dark-700/60">
                    ...
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-gray-700 dark:text-gray-300">
                      ...
                      <button
                        ...
                        type="button"
                        class="mt-2 inline-flex items-center gap-1 rounded-md border border-zo-signal-200 bg-zo-signal-50 px-2 py-1 text-xs font-medium text-zo-signal-700 transition-colors hover:bg-zo-signal-100 disabled:cursor-not-allowed disabled:opacity-60 dark:border-zo-signal-900/60 dark:bg-zo-signal-900/20 dark:text-zo-signal-300 dark:hover:bg-zo-signal-900/30"
              ...
              <div class="grid grid-cols-1 gap-4 p-4 xl:grid-cols-[minmax(0,1fr)_minmax(360px,440px)]">
                <div class="space-y-3">
                  <div class="flex flex-col gap-2 rounded-lg border border-gray-100 bg-gray-50 p-2 dark:border-dark-700 dark:bg-dark-900/30 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="inline-flex rounded-lg bg-white p-1 shadow-sm dark:bg-dark-800">
                      ...
                      <button
                        type="button"
                        class="rounded-md px-3 py-1.5 text-xs font-medium transition-colors"
                        :class="configForm.api_keys_mode === 'replace' ? 'bg-zo-alert-500 text-white shadow-sm' : 'text-gray-600 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-dark-700'"
                        :disabled="configForm.clear_api_key"
                  ...
                  <div class="flex flex-wrap items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                    ...
                    <span v-else-if="pendingDeletedApiKeyCount > 0" class="inline-flex rounded-md bg-zo-alert-50 px-2 py-1 text-zo-alert-700 dark:bg-zo-alert-900/20 dark:text-zo-alert-300">
                      {{ t('admin.riskControl.apiKeyPendingDeleteCount', { count: pendingDeletedApiKeyCount }) }}
                    </span>
                    <span v-if="configForm.api_keys_mode === 'replace'" class="inline-flex rounded-md bg-zo-alert-50 px-2 py-1 text-zo-alert-700 dark:bg-zo-alert-900/20 dark:text-zo-alert-300">
                ...
                <div class="rounded-lg border border-gray-100 bg-gray-50 p-3 dark:border-dark-700 dark:bg-dark-900/30">
                  ...
                  <div v-else class="space-y-2">
                    <div class="space-y-2" :class="apiKeyRowsExpanded ? 'max-h-72 overflow-y-auto pr-1' : ''">
                      <div
                        ...
                        :class="isStoredApiKeyPendingDelete(row) ? 'border-zo-alert-200 opacity-70 dark:border-zo-alert-800/60' : 'border-gray-100 dark:border-dark-700'"
                      >
                        ...
                        <p v-if="row.last_error" class="mt-1.5 rounded-md bg-zo-alert-50 px-2 py-1.5 text-xs leading-5 text-zo-alert-700 dark:bg-zo-alert-900/20 dark:text-zo-alert-300">
                          {{ row.last_error }}
                        </p>
                      </div>
                    </div>
                  ...
                  <div v-if="moderationTestResult" class="mt-4 rounded-lg border border-gray-100 bg-white p-3 dark:border-dark-700 dark:bg-dark-800">
                    <div class="flex items-start justify-between gap-3">
                      ...
                      <span class="inline-flex rounded-full px-2 py-1 text-xs font-medium" :class="moderationTestResult.flagged ? 'bg-red-50 text-red-700 dark:bg-red-900/20 dark:text-red-300' : 'bg-zo-signal-50 text-zo-signal-700 dark:bg-zo-signal-900/20 dark:text-zo-signal-300'">
                    ...
                    <div class="mt-3">
                      ...
                      <div class="h-2 overflow-hidden rounded-full bg-gray-100 dark:bg-dark-700">
                        <div class="h-full rounded-full" :class="moderationTestResult.flagged ? 'bg-red-500' : 'bg-zo-signal-500'" :style="{ width: percentWidth(moderationTestResult.composite_score) }"></div>
                      </div>
                    </div>
...
const keywordNoticeTones = {
  ...
  warning: {
    ...
    toneClass: 'border-zo-alert-200 bg-zo-alert-50 dark:border-zo-alert-900/40 dark:bg-zo-alert-900/20',
    iconClass: 'mt-0.5 flex-shrink-0 text-zo-alert-500 dark:text-zo-alert-300',
    titleClass: 'text-zo-alert-700 dark:text-zo-alert-200',
  },
}
...
const overviewItems = computed<OverviewItem[]>(() => [
  {
    ...
    iconClass: configForm.enabled
      ? 'bg-zo-signal-50 text-zo-signal-600 dark:bg-zo-signal-900/20 dark:text-zo-signal-300'
  ...
  },
  {
    ...
    icon: 'document',
    iconClass: 'bg-zo-alert-50 text-zo-alert-600 dark:bg-zo-alert-900/20 dark:text-zo-alert-300',
  },
])
...
const preBlockMetricItems = computed(() => [
  ...
  },
  {
    ...
    class: 'bg-zo-signal-50 dark:bg-zo-signal-900/10',
    valueClass: 'text-zo-signal-700 dark:text-zo-signal-300',
  },
  {
    key: 'blocked',
  ...
  },
  {
    ...
    class: 'bg-zo-alert-50 dark:bg-zo-alert-900/10',
    valueClass: 'text-zo-alert-700 dark:text-zo-alert-300',
  },
  {
    key: 'latency',
...
const runtimeBadgeClass = computed(() => {
  ...
  return 'bg-zo-signal-50 text-zo-signal-700 dark:bg-zo-signal-900/20 dark:text-zo-signal-300'
})
...
function resultBadgeClass(row: ContentModerationLog): string {
  ...
  if (row.action === 'error' || row.error) return 'bg-zo-alert-100 text-zo-alert-700 dark:bg-zo-alert-900/30 dark:text-zo-alert-300'
  if (row.flagged) return 'bg-pink-100 text-pink-700 dark:bg-pink-900/30 dark:text-pink-300'
  return 'bg-zo-signal-100 text-zo-signal-700 dark:bg-zo-signal-900/30 dark:text-zo-signal-300'
}
...
function workerSlotClass(state: WorkerSlotState): string {
  ...
  if (state === 'idle') {
    return 'border-zo-signal-200 bg-zo-signal-50 text-zo-signal-700 dark:border-zo-signal-900/60 dark:bg-zo-signal-900/20 dark:text-zo-signal-300'
  }
...
function workerDotClass(state: WorkerSlotState): string {
  ...
  if (state === 'idle') return 'bg-zo-signal-500'
...
function apiKeyStatusBadgeClass(statusValue: ContentModerationAPIKeyStatus['status']): string {
  const classes: Record<ContentModerationAPIKeyStatus['status'], string> = {
    ok: 'bg-zo-signal-50 text-zo-signal-700 dark:bg-zo-signal-900/20 dark:text-zo-signal-300',
    error: 'bg-zo-alert-50 text-zo-alert-700 dark:bg-zo-alert-900/20 dark:text-zo-alert-300',
