    ...
    <div class="w-full max-w-md space-y-6">
      ...
      <template v-else>
        ...
        <div class="text-center">
          <div v-if="isSuccess"
            class="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-zo-signal-100 dark:bg-zo-signal-900/30">
            <svg class="h-10 w-10 text-zo-signal-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"
              stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
            </svg>
          </div>
          <div v-else-if="isPending"
            class="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-zo-alert-100 dark:bg-zo-alert-900/30">
            <div class="h-10 w-10 animate-spin rounded-full border-4 border-zo-alert-500 border-t-transparent"></div>
          </div>
          <div v-else
