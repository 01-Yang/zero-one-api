<template>
  <div v-if="hasActiveSubscriptions" class="relative" ref="containerRef">
    ...
    <button
      @click="toggleTooltip"
      class="flex cursor-pointer items-center gap-2 rounded-lg bg-gray-100 px-3 py-1.5 transition-colors duration-200 hover:bg-gray-200 dark:bg-dark-800 dark:hover:bg-dark-700"
      :title="t('subscriptionProgress.viewDetails')"
    >
      <Icon name="creditCard" size="sm" class="text-gray-700 dark:text-gray-300" />
    ...
    <transition name="dropdown">
      <div
        v-if="tooltipOpen"
        class="absolute right-0 z-50 mt-2 w-[340px] overflow-hidden rounded-lg border border-gray-200 bg-white shadow-card dark:border-dark-700 dark:bg-dark-800"
      >
          ...
          >
            ...
            <div class="space-y-1.5">
              ...
              <div
                ...
                class="flex items-center gap-2 rounded-md bg-zo-signal-50 px-2.5 py-1.5 dark:bg-zo-signal-900/20"
              >
...
function getProgressDotClass(sub: UserSubscription): string {
  ...
  if (maxPercentage >= 70) return 'bg-zo-alert-500'
  return 'bg-zo-signal-500'
}
