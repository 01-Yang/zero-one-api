    ...
    <div class="px-6 py-6 space-y-6">
      ...
      <template v-if="notifyEnabled">
        ...
        <div>
          ...
          <p class="mb-2 text-xs text-zo-alert-600 dark:text-zo-alert-400">{{ t('profile.balanceNotify.extraEmailsHint') }}</p>
              ...
              <div class="flex items-center gap-2 shrink-0">
                <template v-if="!entry.verified">
                  ...
                  <template v-else>
                    ...
                    <span class="text-xs text-zo-alert-500">{{ t('profile.balanceNotify.unverified') }}</span>
                  </template>
                </template>
                <span v-else class="text-xs text-zo-signal-500">{{ t('profile.balanceNotify.verified') }}</span>
