<template>
  <BaseDialog :show="show" :title="t('admin.users.groupConfig')" width="wide" @close="$emit('close')">
    <div v-if="user" class="space-y-6">
      <!-- 用户信息头部 -->
      <div class="flex items-center gap-4 rounded-lg border border-gray-200 bg-gray-50 p-5 dark:border-dark-700 dark:bg-dark-800">
      ...
      <div v-else class="space-y-6">
        ...
        <div v-if="publicGroups.length > 0">
          <div class="mb-3 flex items-center gap-2">
            <div class="h-1.5 w-1.5 rounded-full bg-zo-signal-500"></div>
          ...
          <div class="grid gap-3">
            <div
              ...
              :key="config.groupId"
              class="relative overflow-hidden rounded-xl border-2 border-zo-signal-200 bg-zo-signal-50/50 p-4 dark:border-zo-signal-800/50 dark:bg-zo-signal-900/10"
            >
              <div class="flex items-center gap-4">
                <!-- 复选框（禁用状态） -->
                <div class="flex-shrink-0">
                  <div class="flex h-5 w-5 items-center justify-center rounded-md border-2 border-zo-signal-400 bg-zo-signal-500 dark:border-zo-signal-600 dark:bg-zo-signal-600">
